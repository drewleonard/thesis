...
remove_punctuation_helper <- function(x) {
  x <- gsub("#", "\002", x)
  x <- gsub("_", "\003", x)
  x <- gsub("[[:punct:]]+", "", x)
...
remove_punctuation <-
  function (x, preserve_intra_word_dashes = FALSE) {
    if (preserve_intra_word_dashes) {
      ...
      x <- remove_punctuation_helper(x)
      gsub("\001", "-", x, fixed = TRUE)
    } else {
      remove_punctuation_helper(x)
    }
  }
...
corpus <-
  tm::tm_map(corpus,
             tm::content_transformer(remove_punctuation),
