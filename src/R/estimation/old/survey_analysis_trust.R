...
df_survey_small <- df_survey %>%
  ...
  mutate(
    # img*_q2 measures political trust
    img0_trust = as.numeric(img0_q2),
    img1_trust = as.numeric(img1_q2),
    img2_trust = as.numeric(img2_q2),
    img3_trust = as.numeric(img3_q2),
    img4_trust = as.numeric(img4_q2),
    img5_trust = as.numeric(img5_q2),
    img6_trust = as.numeric(img6_q2),
    img7_trust = as.numeric(img7_q2),
    img8_trust = as.numeric(img8_q2),
    img9_trust = as.numeric(img9_q2)
  )
...
# Get Nx2 matrix G
df_merged_small_dfm$race.x[df_merged_small_dfm$race.x == 2] <- 0
G_race <- df_merged_small_dfm %>% select(race.x)
G_race$white <- G_race$race.x
G_race$black <- abs(G_race$race.x - 1)
G_race <- G_race %>% select(white, black)
G_race <- as.matrix(G_race)
...
sibp.rank_1 <- sibp_rank_runs(sibp.search_3_1 , X, 30)

sibp.fit_1 <- sibp.search_3_1[["3"]][["0.5"]][[3]]
sibp_top_words(sibp.fit_1, colnames(X), 30, verbose = TRUE)

sibp.amce_1 <- sibp_amce_temp(sibp.fit_1, X, Y, G = G_race)
sibp_amce_plot(sibp.amce_1, L = 2)

draw_treatment_effects(
  sibp.amce = sibp.amce_1,
...
# Model 2: G for partisanship

# Get Nx2 matrix G
df_merged_small_dfm$partisanship[df_merged_small_dfm$partisanship == 2] <-
  0
G_partisanship <- df_merged_small_dfm %>% select(partisanship)
G_partisanship$democrat <- G_partisanship$partisanship
G_partisanship$republican <- abs(G_partisanship$partisanship - 1)
G_partisanship <- G_partisanship %>% select(democrat, republican)
G_partisanship <- as.matrix(G_partisanship)
...
sibp.rank_2 <- sibp_rank_runs(sibp.search_3_2 , X, 30)

sibp.fit_2 <- sibp.search_3_2[["3"]][["1"]][[3]]
sibp_top_words(sibp.fit_2, colnames(X), 30, verbose = TRUE)

sibp.amce_2 <- sibp_amce_temp(sibp.fit_2, X, Y, G = G_partisanship)
sibp_amce_plot(sibp.amce_2, L = 2)

draw_treatment_effects(
  sibp.amce = sibp.amce_2,
