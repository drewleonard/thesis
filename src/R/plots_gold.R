...
# Load stm model
stm <- readRDS("./rds/interests_age_49.RDS")
...
topics$"primary_topic" <- colnames(topics[,c(1:ncol(stm$theta))])[max.col(topics[,c(1:ncol(stm$theta))],ties.method="first")]
...
full <- left_join(fb, topics,  by=c("AdID" = "AdID"))
...
full %>% 
  drop_na(primary_topic) %>%
  ggplot(aes(x = as.Date(CreationDate), y = as.factor(primary_topic))) + 
  geom_density_ridges() +
...
prepAdSpendBin <- estimateEffect(formula = 1:49 ~ BinAdSpend,
                                 stmobj = stm,
                                 metadata = out$meta, 
                                 uncertainty = "Global")
pdf('~/Documents/thesis/data/figures/effects_bin_adspend.pdf')
plot(prepAdSpendBin,
     covariate = "BinAdSpend", 
     topics = c(2, 21, 37),
     model = stm, 
     method = "difference",
     cov.value1 = "high", 
...
plot(prepAdSpendBin, "BinAdSpend", method = "difference", 
     cov.value1 = "high", 
     cov.value2 = "low", 
     topics = topicNamesDf$TopicNumber,
     verbose.labels = F, 
...
prepAgeBin <- estimateEffect(formula = 1:49 ~ BinAge,
                             stmobj = stm,
                             metadata = out$meta, 
                             uncertainty = "Global")
...
plot(prepAgeBin, "BinAge", method = "difference", 
     cov.value1 = "high", 
     cov.value2 = "low", 
     topics = topicNamesDf$TopicNumber,
     verbose.labels = F, 
...
prepAgeQuant <- estimateEffect(formula = 1:49 ~ Age,
                               stmobj = stm,
                               metadata = out$meta,
                               uncertainty = "Global")

pdf('~/Documents/thesis/data/figures/effects_quant_age.pdf')
plot(prepAgeQuant, "Age", 
     method = "continuous", 
     topics = c(8,37),
     model = stm, 
     printlegend = F, 
